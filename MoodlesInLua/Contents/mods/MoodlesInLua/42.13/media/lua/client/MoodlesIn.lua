...
function ISMoodlesInLua:getIconTexturePath(moodleTranslationName)
    local defaultPath = "media/ui/MIL/Default"
    local iconName = self.iconPaths[tostring(moodleTranslationName)]
...
function ISMoodlesInLua:drawMoodleTooltip(moodle, moodleType, moodleX, moodleY)
    local title = moodle:getMoodleDisplayString(moodleType)
    local description = moodle:getMoodleDescriptionString(moodleType)
...
function ISMoodlesInLua:render()
    ...
    if player and player:getMoodles() then
        ...
        for moodleId = 0, numMoodles - 1 do
            ...
            -- get translated name for icon set
...
if MF ~= nil then
    ...
    -- I added this function because moodles in lua can show FoodEaten on all levels, so it's just copying the code and removing one condition
